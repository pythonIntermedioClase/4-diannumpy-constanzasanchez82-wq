...
def medir_diferencia_vectorizacion(n=1_000_000):
    ...
    print(iva_array)
    #Volver iva_array negativo:
    iva_negativos= iva_array *-1
    
    #Sumar array con iva mas el array negativo:
    suma_arrays= iva_array + iva_negativos
    print(suma_arrays)
    
    pocos_datos = np.array([50,500,5000], dtype=np.float64)
    suma_total= suma_arrays + pocos_datos
...
#Ejercicio 5 elementos de tipo entero, usando funcion for, 
#suma de tipo acumulador y la instruccion sum()
def ejercicio():
    var_acumulador = 0
    lista = [2,3,4,5,6]
    for i in lista:
        var_acumulador += i
    print(var_acumulador)
    ...
    #para saber la posicion y los valores:
    valores =[40,50,60]
...
#Arrays: vectorización. Numpy tipo ndarray
    iva = []
